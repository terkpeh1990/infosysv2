...
def add_document_budget(request,file_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            document_budget = form.save(commit=False)
            document_budget.document_id = file
            document_budget.destination = doc
...
def remove_budget_item(request,item_id):
    ...
    budget.delete()
    messages.error(request,'Budget Item Removed')
    return redirect('dms:add-document_budget',doc.id)
            ...
            if os.path.exists(file_path):           
                ...
                try:
                    for i in dbframe.itertuples():
                        try:
                            staff = User.objects.get(staffid=i.StaffID)
                            DocumentBeneficiary.objects.get_or_create(document_id=file,staff=staff,amount=i.Amount)
